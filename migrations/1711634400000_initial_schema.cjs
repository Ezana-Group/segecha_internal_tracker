...
exports.down = (pgm) => {
  // Drop tables in reverse order if needed, but for initial schema we 
  // often skip 'down' or just drop all.
  pgm.sql(`
    DROP TABLE IF EXISTS system_settings_audit, system_settings, staff_auth, driver_auth, documents, incidents, tyre_logs, maintenance_logs, payroll, invoices, expenses, fuel_logs, journeys, customers, staff, drivers, trailers, trucks, admins, superadmins CASCADE;
  `);
};
