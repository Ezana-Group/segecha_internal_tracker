...
export function Payroll({ data, setData, dark, isMobile, modal, form, setForm, openModal, closeModal, saveItem, delItem, disbursePayroll, truckReg, customerName, driverName }) {
    ...
    const refinedPayroll = allPayees.map(payee => {
        ...
        return {
            ...
            baseSalary: payee.salary || payee.basicSalary || 0,
            allowance: calculatedMileage, // Default to calculated mileage
            deductions: 0,
            _name: payee.name,
            _role: payee.role || (data.drivers?.find(d => d.id === payee.id) ? 'Driver' : 'Staff'),
            _net: Number(payee.salary || payee.basicSalary || 0) + calculatedMileage,
            _base: Number(payee.salary || payee.basicSalary || 0),
