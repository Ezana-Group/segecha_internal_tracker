...
const BackupView = ({ backups, loading, fetchBackups, createManualBackup, restoreFromBackup, downloadBackup, uploadBackup, T, S }) => {
    const sTh = { padding: '12px 14px', borderBottom: `2px solid ${T?.border || 'var(--border-subtle)'}`, color: T?.textDim || 'var(--text-dim)', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: 'left' };
    const sTd = { padding: '12px 14px', borderBottom: `1px solid ${T?.border || 'var(--border-subtle)'}`, color: T?.text || 'var(--text-secondary)', fontSize: 13 };

    React.useEffect(() => { fetchBackups(); }, [fetchBackups]);

    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ display: 'flex', gap: 12 }}>
                <Button variant="primary" onClick={createManualBackup} disabled={loading}>
                    <Download size={16} /> Create New Backup
                </Button>
                <label style={{ 
                    display: 'inline-flex', cursor: 'pointer', alignItems: 'center', gap: 8,
                    background: 'var(--bg-card)', color: 'var(--text-primary)', padding: '10px 18px',
                    borderRadius: 8, fontWeight: 600, fontSize: 13, border: '1px solid var(--border-subtle)'
                 }}>
            ...
            <Card style={{ padding: 0, overflow: 'hidden' }}>
                <div className="table-container" style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                            <tr style={{ background: 'var(--bg-main)' }}>
                                <th style={sTh}>Backup Name</th>
                                <th style={sTh}>Date Created</th>
                                <th style={sTh}>Size</th>
                                <th style={{ ...sTh, textAlign: 'right' }}>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading && backups.length === 0 ? (
                                <tr><td colSpan={4} style={{ padding: '40px 20px', textAlign: 'center' }}>Loading backups...</td></tr>
                            ) : backups.length === 0 ? (
                                <tr><td colSpan={4} style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--text-dim)' }}>No backups found on server.</td></tr>
                            ) : (
                                backups.map((b, i) => (
                                    <tr key={i} className="hover-scale">
                                        <td style={sTd}><div style={{ fontWeight: 700 }}>{b.name}</div></td>
                                        <td style={sTd}>{new Date(b.timestamp).toLocaleString('en-KE')}</td>
                                        <td style={sTd}>{b.size}</td>
                                        <td style={{ ...sTd, textAlign: 'right', display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                                            <Button variant="ghost" size="sm" onClick={() => downloadBackup(b.name)} title="Download">
                                                <Download size={14} />
                                            </Button>
                                            <Button variant="ghost" size="sm" onClick={() => restoreFromBackup(b.name)} style={{ color: '#f97316' }} title="Restore">
        ...
        return (
            <div className="page-shell" style={{ maxWidth: 840, padding: "40px 20px" }}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 40 }}>
                    <div 
                        onClick={() => setView('new')}
                        style={{ background: view === 'new' ? "#f97316" : "var(--bg-card)", color: view === 'new' ? "white" : "var(--text-dim)", padding: "8px 20px", borderRadius: 20, fontSize: 13, fontWeight: 700, cursor: "pointer", boxShadow: view === 'new' ? "0 2px 8px rgba(249,115,22,0.3)" : "none", border: view === 'new' ? "none" : "1px solid var(--border-subtle)" }}>
                        New Import
                    </div>
                    <div 
                        onClick={() => setView('history')}
                        style={{ background: view === 'history' ? "#f97316" : "var(--bg-card)", color: view === 'history' ? "white" : "var(--text-dim)", padding: "8px 20px", borderRadius: 20, fontSize: 13, fontWeight: 700, cursor: "pointer", boxShadow: view === 'history' ? "0 2px 8px rgba(249,115,22,0.3)" : "none", border: view === 'history' ? "none" : "1px solid var(--border-subtle)" }}>
                        Import History
                    </div>
                    <div 
                        onClick={() => setView('backup')}
                        style={{ background: view === 'backup' ? "#f97316" : "var(--bg-card)", color: view === 'backup' ? "white" : "var(--text-dim)", padding: "8px 20px", borderRadius: 20, fontSize: 13, fontWeight: 700, cursor: "pointer", boxShadow: view === 'backup' ? "0 2px 8px rgba(249,115,22,0.3)" : "none", border: view === 'backup' ? "none" : "1px solid var(--border-subtle)" }}>
                        System Backup
                    </div>
                </div>
